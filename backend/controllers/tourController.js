...
const createTour = async (req, res) => {
  try {
    const { title, city, desc, maxGroupSize, photo, address, price, distance } = req.body;
...
const updateTour = async (req, res) => {
  try {
    const tourId = req.params.id;
    const { title, city, photo, desc, maxGroupSize, address, price, distance } = req.body;

    const newTour = await Tour.findByIdAndUpdate(tourId, { title, city, photo, desc, maxGroupSize, address, distance, price }, { new: true });
...
const getTourBySearch = async (req, res) => {
  try {
    ...
    if (!searchTerm && (isNaN(minPrice) || isNaN(maxPrice))) {
      return res.status(400).json({ message: 'Search term or price range is required' });
    }
...
export { getAllTours, getSingleTour, createTour, updateTour, deleteTour, getTourBySearch, getFeaturedTour, getTourCount };
